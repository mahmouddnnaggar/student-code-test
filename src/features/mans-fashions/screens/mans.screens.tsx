...
import getMansFashion from '../server/mans.actions';
import { setMansProducts } from '../store/mans.slice';
...
export default function MansFashionScreen() {
  const dispatch = useAppDispatch();
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const { products } = useSelector((state: RootState) => state.mansFashion);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const response = await getMansFashion();
        dispatch(setMansProducts(response.data));
      } catch (err: any) {
        setError("Failed to load Men's Fashion products.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // دالة الإضافة للكارت الاحترافية
  const handleAddToCart = async (id: string) => {
    try {
      const response = await addProducToCart({ productId: id });
      if (response.status === "success") {
        toast.success(response.message || "Added to cart! 🛒");
        dispatch(getLoggedUserCart()); // تحديث الرقم في الناف بار فوراً
      }
    } catch (error) {
      ...
      <div className="container mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 text-center">
          <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight">
             <span className="text-green-600">Men's</span> Fashion
          </h1>
          <p className="mt-4 text-xl text-gray-500 max-w-2xl mx-auto">
            Explore our latest collection of premium menswear and accessories.
          </p>
        </div>
